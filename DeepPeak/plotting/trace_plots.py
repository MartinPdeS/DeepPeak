...
def plot_detection_comparison(record: metrics.TraceRecord, **kwargs) -> plt.Figure:
    ...
    plot_config = kwargs.pop("config", None)
    if plot_config is not None:
        if not isinstance(plot_config, PlotConfig):
            raise TypeError("config must be a PlotConfig instance.")
        kwargs.setdefault("figsize", plot_config.figsize)
        kwargs.setdefault("show", plot_config.show)
        kwargs.setdefault("close", plot_config.close)
        kwargs.setdefault("dpi", plot_config.dpi)
        if plot_config.title is not None:
            kwargs.setdefault("title", plot_config.title)
