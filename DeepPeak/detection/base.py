...
class BaseAmplitudeSolver:
    ...
    def plot(
        self, true_amplitudes: NDArray[np.float64], sample_index: int = 0
    ) -> plt.Figure:
        """
        ...
        if getattr(self, "last_centers_", None) is not None:
            ...
            labels = [
                f"μ{j}={centers[j]:.3f}" if j < centers.shape[0] else f"μ{j}"
                for j in range(A)
            ]
        else:
    ...
    @staticmethod
    def _response_matrix_from_centers(
        centers_2d: NDArray[np.float64], sigma: float
    ) -> NDArray[np.float64]:
        r"""
