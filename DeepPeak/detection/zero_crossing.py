...
class ZCSingleResult:
    ...
    def summary(self) -> dict:
        return {
            ...
            "require_negative_curvature": bool(
                self.detector.require_negative_curvature
            ),
        }
    ...
    def plot(self, *, title: Optional[str] = None) -> plt.Figure:
        ...
        fig, (ax1, ax2) = plt.subplots(
            2, 1, figsize=(7.5, 5.8), sharex=True, gridspec_kw={"height_ratios": [2, 1]}
        )
    ...
    def plot(
        ...
        *,
        ...
        fig, axes = plt.subplots(
            nrows, ncols, figsize=(6.5 * ncols, 2.8 * nrows), squeeze=False
        )
        ...
        for k, (example_number, ax) in enumerate(zip(batch_index, axes_flat)):
            ...
            ax.plot(
                self.time_samples,
                self.smoothed_signals[example_number],
                label="smoothed",
            )
            ax.axhline(
                self.threshold_used[example_number],
                linestyle="--",
                alpha=0.7,
                label="threshold" if k == 0 else None,
            )
            ...
            if ground_truth is not None:
                ...
                for idx, ground_truth_time in enumerate(
                    ground_truth_times[np.isfinite(ground_truth_times)]
                ):
                    ax.axvline(
                        ground_truth_time,
                        color="black",
...
class ZeroCrossing:
    ...
    def run(
        ...
        if keep.size > self.maximum_number_of_peaks:
            top = np.argpartition(y_s[keep], -self.maximum_number_of_peaks)[
                -self.maximum_number_of_peaks :
            ]
            keep = keep[top]
        keep = np.sort(keep)
    ...
    def run_batch(
        ...
        cand_lists = [
            np.where(mask[b])[0] + 1 for b in range(B)
        ]  # list of 1D int arrays
        ...
        for b in range(B):
            ...
            if kept.size > self.maximum_number_of_peaks:
                top = np.argpartition(y_s[b, kept], -self.maximum_number_of_peaks)[
                    -self.maximum_number_of_peaks :
                ]
                kept = kept[top]
