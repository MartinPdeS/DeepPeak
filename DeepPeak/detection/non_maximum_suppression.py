...
class SingleResult:
    ...
    def summary(self) -> dict:
        ...
        return {
            ...
            "max_matched": (
                float(np.max(self.matched_filter_output))
    ...
    def plot(
        ...
        *,
        ...
        if show_kernel and peaks_t.size and self.gaussian_kernel is not None:
            ...
            kernel_proxy = plt.Line2D(
                [0], [0], linestyle="--", color=color_kernel, linewidth=1.4, alpha=0.85
            )
        else:
            kernel_proxy = None
        ...
        ax.set(
            title=f"Detected peaks: {self.number_of_peaks}",
            xlabel="t",
            ylabel="amplitude",
        )
        return fig
...
class BatchResult:
    ...
    def _plot_kernel_overlay(self, ax, example_number: int) -> None:
        ...
        idxs = (
            self.peak_indices[example_number] if hasattr(self, "peak_indices") else None
        )
        if idxs is None:
            return
        ...
        for i in idxs[valid]:
            ...
            y_k = alpha * k
            ax.plot(
                t_k, y_k, linestyle="--", linewidth=1.4, alpha=0.85, color=color_kernel
            )
    ...
    def _plot_signal_and_filter(self, ax, example_number: int) -> None:
        ...
        ax.plot(
            self.time_samples,
            self.matched_filter_output[example_number],
    ...
    def _plot_true_positions(
        self, ax, example_number: int, true_position: np.ndarray
    ) -> None:
