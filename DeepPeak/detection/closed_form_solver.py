...
from .base import BaseAmplitudeSolver
...
class BatchedResults:
    ...
    def compare_plot(
        self, true_amplitudes: NDArray[np.float64], ncols: int = 2, max_plots: int = 6
    ) -> plt.Figure:
        """
        ...
        for i in range(num_figure):
            ax[i].bar(
                np.arange(len(true_amplitudes[i])) - 0.2,
                true_amplitudes[i],
                width=0.4,
                label="True",
                color="C0",
...
class ClosedFormSolver(BaseAmplitudeSolver):
    ...
    def run(
        ...
        if A == 1:
            ...
            raise ValueError(
                f"Number of estimated peaks must be 1 - 5, got {A}. Higher orders not implemented."
            )
    ...
    def _solve_order2(
        self, G: NDArray[np.float64], Yc: NDArray[np.float64]
    ) -> NDArray[np.float64]:
        """
    ...
    def run_batch(
        self, centers: NDArray[np.float64], center_samples: NDArray[np.float64]
    ) -> NDArray[np.float64]:
        """
        ...
        if centers.shape != center_samples.shape:
            raise ValueError(
                "centers and center_samples must have the same shape (B, A)."
            )
        ...
        match number_of_estimated_peaks:
            ...
            case 3:
                r12, r13, r23 = G[:, 0, 1], G[:, 0, 2], G[:, 1, 2]
                det = (1 + 2 * r12 * r13 * r23 - r12**2 - r13**2 - r23**2).clip(
                    min=self.eps
                )
            ...
            case 4:
                ...
                det3 = (1 + 2 * r12 * r13 * r23 - r12**2 - r13**2 - r23**2).clip(
                    min=self.eps
                )
            ...
            case _:
                raise ValueError(
                    f"Number of estimated peaks must be between 1 and 4, got {number_of_estimated_peaks}."
                )
