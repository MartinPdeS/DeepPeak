...
@dataclass
class WaveNet(BaseDeconvolver):
    """
    WaveNet-style 1D neural deconvolver for pulse-trace reconstruction.
    ...
    output_activation: str | None
        Activation for the final 1x1 output head. Use ``"sigmoid"`` for
        probability-like masks and ``"linear"`` or ``None`` for additive pulse
        trace regression.
    ...
    def build(self) -> tf.keras.Model:
        ...
        for i in range(self.num_dilation_layers):
            ...
            if self.dropout_rate > 0.0:
                h = layers.SpatialDropout1D(self.dropout_rate, name=f"dropout_{i}")(h)
        ...
        self.model = models.Model(
            inputs=inputs, outputs=outputs, name="WaveNetDeconvolver"
        )
        self.model.compile(
