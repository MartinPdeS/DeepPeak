    ...
    def plot(
        ...
        *,
        ...
        show_kernel : bool, optional
            If True, overlay the Gaussian kernel (unit-energy) centered at each detected peak,
            scaled so max(kernel) equals the raw peak amplitude at that peak.
        """
        if indices is None:
        ...
        # Precompute kernel overlay parts once
        k = self.gaussian_kernel if show_kernel else None
        if show_kernel and k is not None and k.size > 0:
            K = k.size
            half = (K - 1) // 2
            dt = float(self.time_samples[1] - self.time_samples[0])
            kmax = float(np.max(np.abs(k))) if np.any(k) else 1.0
        ...
        for panel_idx, (example_number, ax) in enumerate(zip(batch_index, axes_flat)):
            ...
            # kernel overlay
            if kernel_proxy is not None and idxs is not None:
                valid = idxs >= 0
                for i in idxs[valid]:
                    ...
                    t_k = m + (np.arange(-half, half + 1, dtype=float) * dt)
                    y_k = alpha * k
                    ax.plot(t_k, y_k, linestyle="--", linewidth=1.4, alpha=0.85, color=kernel_proxy.get_color())

            if ground_truth is not None:
                gt = ground_truth[example_number]
                for j, gt_t in enumerate(gt[np.isfinite(gt)]):
                    ax.axvline(gt_t, color="black", linestyle=":", alpha=0.6, label="ground truth" if j == 0 else None)
            ...
            # first panel: build a clean legend including kernel proxy if present
            if panel_idx == 0:
                handles, labels = ax.get_legend_handles_labels()
                if kernel_proxy is not None:
                    handles.append(kernel_proxy)
