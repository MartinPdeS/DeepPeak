...
def test_output_shapes(default_params):
    ...
    signals, amplitudes, positions, widths, x_values, num_peaks = generate_signal_dataset(**default_params)
...
def test_signal_types(default_params):
    ...
    for signal_type in ["gaussian", "lorentzian", "bessel", "square", "asym_gaussian", "dirac"]:
        ...
        signals, _, _, _, _, _ = generate_signal_dataset(**default_params)
        assert signals.shape == (default_params["n_samples"], default_params["sequence_length"])
...
def test_random_seed_consistency(default_params):
    ...
    signals_1, amplitudes_1, positions_1, _, _, _ = generate_signal_dataset(**default_params)
    signals_2, amplitudes_2, positions_2, _, _, _ = generate_signal_dataset(**default_params)
...
def test_categorical_peak_count(default_params):
    ...
    signals, _, _, _, _, num_peaks = generate_signal_dataset(**default_params)
...
def test_zero_noise(default_params):
    ...
    signals_1, _, _, _, _, _ = generate_signal_dataset(**default_params)
    signals_2, _, _, _, _, _ = generate_signal_dataset(**default_params)
...
def test_edge_case_zero_peaks(default_params):
    ...
    default_params["noise_std"] = 0
    signals, amplitudes, _, _, _, _ = generate_signal_dataset(**default_params)
...
def test_edge_case_extreme_values(default_params):
    ...
    _, amplitudes, _, widths, _, _ = generate_signal_dataset(**default_params)
...
def test_width_position_amplitude_scalar_interpretation(default_params):
    ...
    _, amplitudes, positions, widths, _, _ = generate_signal_dataset(**default_params)
