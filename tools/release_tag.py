#!/usr/bin/env python3
"""Create a DeepPeak release commit and annotated Git tag.

The tag is the canonical package version. This helper aligns the project
metadata, Conda recipe, and generated source version file before committing
and tagging the release. It intentionally does not push to a remote.
"""
...
VERSION_FILE = ROOT / "DeepPeak" / "_version.py"
...
def validate_tag(tag: str) -> str:
    ...
    if match is None:
        raise ValueError("tag must use the form vMAJOR.MINOR.PATCH, for example v0.2.0")
...
def generate_version_file(version: str) -> None:
    ...
    if not VERSION_FILE.exists():
        raise RuntimeError("SCM versioning did not generate DeepPeak/_version.py")
...
def create_release(tag: str, version: str) -> None:
    ...
    run(
        "git", "add", "conda.recipe/meta.yaml", "pyproject.toml", "DeepPeak/_version.py"
    )
...
def parse_arguments() -> argparse.Namespace:
    ...
    parser.add_argument("tag", help="annotated release tag, for example v0.2.0")
